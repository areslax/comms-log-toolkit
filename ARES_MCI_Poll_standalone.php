...
$stid = (empty($_GET['sid']) || !is_numeric($_GET['sid'])) ? 0:$_GET['sid'];
...
<tr>
<th style="font-size:1.3em">ARES MCI Poll<br><div contenteditable='true' class='location' style='position:relative;left:50%;margin-left:-250px;border:solid 1px lightgrey;padding:2px;width:500px;height:20px;text-align_center;font-weight:normal;font-size:0.8em;<?=$loccolor?>' onfocus='this.style.color="black";this.innerHTML=""'><?=$longloc?></div></th></tr>
<tr>
<td align=center>
...
function initAuto() {
	...
	jQuery(".location").autocomplete({
		...
		select: function(event, ui) {
			event.preventDefault();
			jQuery(this).val(ui.item.label);
			location.href="ARES_MCI_Poll_standalone.php?fld="+ui.item.label;
		}
	});
