...
if (!empty($iid)) {
	//single incident
	$oq = $conn->prepare("select Members.m_id,m_callsign,m_status,m_fname,m_lname,Operators.i_id,i_name,Operators.l_id,l_tactical,l_name,Operators.nc_id,nc_callsign,s_title from Members left outer join Operators on Operators.m_id=Members.m_id left outer join Locations on Locations.l_id=Members.m_prestage_lid left outer join Net_Controls on Net_Controls.nc_id=Operators.nc_id left outer join Status_Codes on Status_Codes.s_id=Members.m_status where i_id=:iid");
	$oq->execute(array(':iid'=>$iid));
	$labl = "Involved in this Incident";
}
else if (!empty($lid)) {
	//single location
	$oq = $conn->prepare("select Members.m_id,m_callsign,m_status,m_fname,m_lname,m_prestage_lid,Operators.i_id,i_name,Operators.l_id,l_tactical,l_name,Operators.nc_id,s_title,s_data,s_color from Members left outer join Operators on Operators.m_id=Members.m_id left outer join Incidents on Incidents.i_id=Operators.i_id left outer join Locations on Locations.l_id=Operators.l_id left outer join Status_Codes on Status_Codes.s_id=Members.m_status where Locations.l_id=:lid");
	$oq->execute(array(':lid'=>$lid));
	$labl = "Deploys to this Location";
}
else if (!empty($ncid)) {
	//single net control
	$oq = $conn->prepare("select Members.m_id,m_callsign,m_status,m_fname,m_lname,m_prestage_lid,Operators.i_id,i_name,Operators.l_id,l_tactical,l_name,Operators.nc_id,s_title,s_data,s_color from Members left outer join Operators on Operators.m_id=Members.m_id left outer join Incidents on Incidents.i_id=Operators.i_id left outer join Locations on Locations.l_id=Operators.l_id left outer join Status_Codes on Status_Codes.s_id=Members.m_status where Net_Controls.nc_id=:ncid");
...
foreach($orow as $o) {
	...
	if (empty($o['l_id'])) {
		$lq = $conn->prepare("select l_tactical,l_name,l_gps from Locations where l_id=:pid limit 1");
	...
	$ops[$cs]['netcontrolname'] = $o['nc_callsign'];
...
foreach($ops as $cs => $data) {
	echo "<tr><td align=left title='".$data['locationname']."' style='cursor:pointer'>".$cs."</td><td align=left><a href='ARES_Member_Manage.php?mid=".$data['mid']."&admin=".$_GET['admin']."' target='_blank' title='Click to view ".$cs." Member Details'>".$data['name']."</a></td><td style='background-color:".$data['statusclr'].";cursor:pointer;' title='".$data['statustxt']."' onclick=\"showStatusEdit(".$data['mid'].",'".$cs.": ".$data['name']."',".$data['statusid'].")\">".$data['status']."</td><th cellpadding=0><a href='https://maps.google.com/?q=".$data['locationgps']."' target='_blank' title='".$data['locationname']."'><img src='images/icon-google-maps.svg' width=14 border=0 style='opacity:.3'></a></th><th cellpadding=0><a href='https://maps.google.com/?q=".$data['netcontrolgps']."' target='_blank' title='".$data['netcontrolcall']."'><img src='images/icon-google-maps.svg' width=14 border=0i style='opacity:.3'></a></th></tr>\n";
}
?>
</table>
<!--/div-->
